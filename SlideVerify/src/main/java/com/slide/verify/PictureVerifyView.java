...
class PictureVerifyView extends AppCompatImageView {
    ...
    private Bitmap verityBlock;        //拼图缺块Bitmap
    private Path blockShape;            //拼图缺块形状
    private final Paint bitmapPaint;         //绘制拼图缺块的画笔
    private final Paint shadowPaint;         //绘制拼图缺块阴影的画笔
    ...
    public PictureVerifyView(Context context, AttributeSet attrs, int defStyleAttr) {
        ...
        mStrategy = new DefaultStrategy(context);
    ...
    private void initDrawElements(){
        if (shadowInfo == null) {
            ...
            if (mMode == SlideVerifyView.MODE_BAR) {
    ...
    protected void onDraw(Canvas canvas) {
        ...
        if (mState == STATE_MOVE || mState == STATE_IDEL || mState == STATE_DOWN || mState == STATE_UNACCESS) {
            canvas.drawBitmap(verityBlock, blockInfo.left, blockInfo.top, bitmapPaint);
        }
    ...
    void down() {
        ...
        mState = STATE_DOWN;
        blockInfo.left = (int) (0 / 100f * (getWidth() - blockSize));
        invalidate();
    }
    ...
    void setCaptchaStrategy(Strategy strategy) {
    ...
    public void setBitmap(Bitmap bitmap) {
        ...
        this.blockInfo = null;
        this.shadowInfo = null;
        this.verityBlock.recycle();
        this.verityBlock = null;
        setImageBitmap(bitmap);
    }

    void setMode(@SlideVerifyView.Mode int mode) {
        this.mMode = mode;
        this.blockShape = null;
        this.blockInfo = null;
        this.shadowInfo = null;
    ...
    public boolean dispatchTouchEvent(MotionEvent event) {
        ...
        if (event.getAction() == MotionEvent.ACTION_DOWN && mMode == SlideVerifyView.MODE_NO_BAR) {
    ...
    public boolean onTouchEvent(MotionEvent event) {
        if (mMode == SlideVerifyView.MODE_NO_BAR && verityBlock != null && mTouchEnable) {
            float x = event.getX();
            float y = event.getY();
